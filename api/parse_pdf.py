...
# ============================================
# Text-based PI Parser
# ============================================
def parse_pi(text: str) -> PIInfo:
    ...
    return PIInfo(
        pi_number=pi_number, pi_date=pi_date, from_company=from_company,
        to_company=to_company, contact=contact, email=email,
        products=products, total_qty=total_qty, total_amount=total_amount
    )

# ============================================
# Text-based CI Parser - Smart EAN detection
# ============================================
def parse_ci(text: str) -> FactoryCI:
    lines = [l.strip() for l in text.split('\n') if l.strip()]
    ...
    items = extract_ci_items(text)

    return FactoryCI(
        invoice_no=invoice_no, invoice_date=invoice_date, pi_no=pi_no,
        sold_to=sold_to, delivered_to=delivered_to, asn=asn, items=items,
        total_qty=sum(it.qty for it in items),
        total_amount=sum(it.amount for it in items)
    )

def extract_ci_items(text: str) -> List[CIItem]:
    result = []

    # Find ALL 13-digit EANs - even with letter prefixes
    # Use [​​[0-9]a-zA-Z] as boundary to handle T6941565984197
    all_eans = set()
    # First pass: find standalone 13-digit sequences
    for m in re.finditer(r'[^\d](\d{13})(?!\d)', text):
        all_eans.add(m.group(1))
    # Second pass: find 13-digit sequences at start of fragments
    for m in re.finditer(r'[a-zA-Z](\d{13})(?!\d)', text):
        all_eans.add(m.group(1))
    # Third pass: find clean 13-digit sequences
    for m in re.finditer(r'\b(\d{13})\b', text):
        all_eans.add(m.group(1))

    for ean in all_eans:
        if any(r.ean == ean for r in result):
            continue

        # Find position of EAN in text
        pos = text.find(ean)
        if pos < 0: continue
        ...
        if not description:
            m = re.search(r'(Remote\s+Controller[\w\/\s]*|Intelligent\s+Battery[\w\/\s]*|FPV\s+Drone[\w\/\s]*|DJI\s+Air[\w\/\s]*|DJI\s+Matrice[\w\/\s]*|DJI\s+Mavic[\w\/\s]*)', after, re.I)
        ...
        if ean and qty > 0:
            result.append(CIItem(
                item_no=str(len(result) + 1),
                part_no=part_no, pi_no=pi_no, ean=ean,
                description=description, hs_code=hs_code, origin=origin,
                qty=qty, uom="PCS", unit_price=unit_price, amount=amount
            ))

    return result

# ============================================
# Text-based PL Parser
# ============================================
def parse_pl(text: str) -> FactoryPL:
    lines = [l.strip() for l in text.split('\n') if l.strip()]
    ...
    return FactoryPL(
        invoice_no=invoice_no, asn=asn, date=date, pi_no=pi_no,
        items=items,
...
def extract_pl_items(text: str) -> List[PLItem]:
    result = []
    ...
    for m in re.finditer(r'([A-Z]+\d+[A-Z]?\d+)-(\d+)(?:~(\d+))?', text):
        ...
        if any(r.case_no == case_no for r in result):
            continue
        ...
        if ean and qty > 0:
            result.append(PLItem(
                case_no=case_no, part_no=part_no, ean=ean,
                description=description, qty=qty, uom="PCS",
                gross_weight=gw, net_weight=nw, size=size, volume=volume,
                num_cases=num_cases
            ))
